...
p <- jhu %>% genPlotComps(
  geo_level = "country", min_thresh = 1, per_capita = TRUE,
...
```{r message=FALSE, warning=FALSE}
p <- jhu %>% genPlotComps(
  geo_level = "country", min_thresh = 10, per_capita = FALSE,
  max_days_since = 30, smooth_plots = TRUE, scale_to_fit = TRUE)
suppressWarnings(ggplotly(p))
```
...
p <- covtrack %>% genPlotComps(
  geo_level = "state", min_thresh = 1, per_capita = TRUE,
  max_days_since = 30, smooth_plots = TRUE, scale_to_fit = TRUE)
suppressWarnings(ggplotly(p))
```
...
```{r message=FALSE, warning=FALSE}
p <- covtrack %>% genPlotComps(
  geo_level = "state", min_thresh = 5, per_capita = FALSE,
  max_days_since = 30, smooth_plots = TRUE, scale_to_fit = TRUE)
suppressWarnings(ggplotly(p))
```
